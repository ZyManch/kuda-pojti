...
foreach ($model->maps as $map):?>
    ...
    <?php endforeach;?>
    <?php echo CHtml::button('Добавить', array('onclick' => '$($("#blank'.$map->id.'").html().split("%").join(Math.round(Math.random()*1000000))).insertBefore(this)'));?>
<?php endforeach; ?>
 <?php echo CHtml::submitButton('Сохранить');?>
<?php $this->endWidget(); ?>
<?php foreach ($model->maps as $map):?>
<div id="blank<?php echo $map->id;?>" style="display: none">
    <?php
    $work = new Work();
    $work->maps_id = $map->id;
    ?>
